...
if "claude_analysis_html" not in st.session_state:
    st.session_state["claude_analysis_html"] = (
        f'Em {DATA_ATUAL["dias_periodo"]} dias, você perdeu '
        f'<span class="hi-green">{fmt_num(abs(DATA_ATUAL["delta_peso_total"]), 1)} kg</span> e reduziu gordura em '
        f'<span class="hi-green">{fmt_num(abs(delta_gordura), 1)} pp</span>. '
        f'Músculo subiu <span class="hi-green">+{fmt_num(delta_musculo, 1)} kg</span> — recomposição real acontecendo.'
        f'<br><br>'
        f'<span class="hi-red">Ritmo de gordura está lento</span>: no ritmo atual, a meta ainda está distante. '
        f'Para encurtar esse prazo, você precisa acelerar a queda de gordura sem perder massa magra.'
        f'<br><br>'
        f'<span class="hi-gold">Foco:</span> treino intenso, proteína alta e constância calórica.'
    )
...
if st.button("✦ Analisar agora com Claude", use_container_width=True):
    prompt = f"""
Você é um analista de composição corporal.

Responda em português do Brasil.
Seja direto, claro e útil.
Escreva em HTML simples, usando apenas:
<br>
<span class="hi-green">...</span>
<span class="hi-red">...</span>
<span class="hi-gold">...</span>
...
- Gordura visceral: {DATA_ATUAL["visceral"]}
- Proteína: {DATA_ATUAL["proteina"]} %
- Meta de peso: {DATA_ATUAL["meta_peso"]} kg
- Meta de gordura: {DATA_ATUAL["meta_gordura"]} %
- Peso inicial: {DATA_ATUAL["peso_inicial"]} kg
- Gordura inicial: {DATA_ATUAL["gordura_inicial"]} %
- Músculo inicial: {DATA_ATUAL["musculo_inicial"]} kg
- Delta de peso no período: {DATA_ATUAL["delta_peso_total"]} kg
- Delta de gordura no período: {delta_gordura:.1f} pp
- Delta de músculo no período: {delta_musculo:.1f} kg
- Dias avaliados: {DATA_ATUAL["dias_periodo"]}
